"""
Training script for the Transformer-SIREN DeepReach model.

This is a modified version of run_experiment.py that uses SirenTransformerBVPNet
instead of SingleBVPNet. All other training logic remains identical.

Usage (same as run_experiment.py, with extra --num_heads argument):
    conda run -n deepreach python run_experiment_transformer.py \
        --mode all \
        --experiment_name my_transformer_experiment \
        --num_heads 4 \
        ... (same args as run_experiment.py)
"""
...
from utils import dataio, losses, modules_transformer
...
if (mode == "all") or (mode == "train"):
    ...
    # model options (adapted for Transformer-SIREN)
    p.add_argument(
        "--model",
        type=str,
        default="sine",
        ...
        help="Type of model (kept for compatibility, not used by transformer).",
    )
    p.add_argument(
        "--model_mode",
        type=str,
    ...
    p.add_argument(
        ...
        default=512,
        required=False,
        help="Hidden features (d_model) per layer.",
    )
    p.add_argument(
        "--num_heads",
        type=int,
    ...
    p.add_argument(
        ...
        default=30.0,
        required=False,
        help="SIREN frequency parameter omega_0.",
    )
    p.add_argument(
        "--deepreach_model",
        type=str,
    ...
    p.add_argument(
        "--CSL_lr", type=float, default=2e-5, help="The learning rate used for CSL"
    )
    p.add_argument(
        "--CSL_dt",
        type=float,
...
if (mode == "all") or (mode == "train"):
    ...
    if os.path.exists(experiment_dir):
        overwrite = input(
            "The experiment directory %s already exists. Overwrite? (y/n)"
            % experiment_dir
        )
        if not (overwrite == "y"):
            print("Exiting.")
...
# === KEY CHANGE: Use SirenTransformerBVPNet instead of SingleBVPNet ===
num_heads = getattr(orig_opt, "num_heads", 4)
omega_0 = getattr(orig_opt, "omega_0", 30.0)
model = modules_transformer.SirenTransformerBVPNet(
    in_features=dynamics.input_dim,
    out_features=1,
    hidden_features=orig_opt.num_nl,
    num_heads=num_heads,
    num_layers=orig_opt.num_hl,
    omega_0=omega_0,
)
model.to(opt.device)
