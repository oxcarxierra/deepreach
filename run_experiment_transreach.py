...
from utils import dataio, losses, modules_transreach


def configure_attention_backend_for_higher_order_grads():
    """Disable fused SDPA kernels that do not support double backward."""
...
if (mode == "all") or (mode == "train"):
    ...
    p.add_argument("--device", type=str, default="auto", required=False, help="Device to use: auto, cuda:0, mps, cpu")
    ...
    # model options
    p.add_argument("--model", type=str, default="sine", required=False, choices=["sine", "tanh", "sigmoid", "relu"], help="Unused (kept for CLI compatibility).")
    p.add_argument("--model_mode", type=str, default="mlp", required=False, choices=["mlp", "rbf", "pinn"], help="Unused (kept for CLI compatibility).")
    p.add_argument("--num_hl", type=int, default=3, required=False, help="Unused (kept for CLI compatibility).")
    p.add_argument("--num_nl", type=int, default=256, required=False, help="Embedding width for TransReach.")
    p.add_argument("--deepreach_model", type=str, default="exact", required=False, choices=["exact", "diff", "vanilla"], help="deepreach model")

    # TransReach-specific options
    p.add_argument("--pseudo_steps", type=int, default=5, required=False, help="Pseudo-sequence length k.")
    p.add_argument("--pseudo_dt", type=float, default=1e-3, required=False, help="Pseudo-sequence step size delta_t.")
    p.add_argument("--num_heads", type=int, default=2, required=False, help="Number of attention heads.")
    p.add_argument("--num_encoder_layers", type=int, default=1, required=False, help="Number of encoder layers.")
    p.add_argument("--num_decoder_layers", type=int, default=1, required=False, help="Number of decoder cross-attention layers.")
    p.add_argument("--ffn_expansion", type=int, default=2, required=False, help="FFN expansion factor.")
    p.add_argument("--dropout", type=float, default=0.0, required=False, help="Dropout in attention/FFN blocks.")
    p.add_argument("--use_layernorm", action="store_true", default=False, required=False, help="Enable LayerNorm in transformer blocks.")
...
configure_attention_backend_for_higher_order_grads()
...
model = modules_transreach.TransReachBVPNet(
    in_features=dynamics.input_dim,
    out_features=1,
    hidden_features=orig_opt.num_nl,
    num_heads=orig_opt.num_heads,
    num_encoder_layers=orig_opt.num_encoder_layers,
    num_decoder_layers=orig_opt.num_decoder_layers,
    ffn_expansion=orig_opt.ffn_expansion,
    pseudo_steps=orig_opt.pseudo_steps,
    pseudo_dt=orig_opt.pseudo_dt,
    tmax=orig_opt.tMax,
    dropout=orig_opt.dropout,
    use_layernorm=orig_opt.use_layernorm,
)
model.to(opt.device)
...
if (mode == "all") or (mode == "train"):
    if dynamics.loss_type == "brt_hjivi":
        loss_fn = losses.init_transreach_brt_hjivi_loss(
            dynamics, orig_opt.minWith, orig_opt.dirichlet_loss_divisor
        )
    elif dynamics.loss_type == "brat_hjivi":
        loss_fn = losses.init_transreach_brat_hjivi_loss(
            dynamics, orig_opt.minWith, orig_opt.dirichlet_loss_divisor
        )
    else:
    ...
    experiment.train(
        ...
        use_CSL=orig_opt.use_CSL,
        CSL_lr=orig_opt.CSL_lr,
        CSL_dt=orig_opt.CSL_dt,
        epochs_til_CSL=orig_opt.epochs_til_CSL,
        num_CSL_samples=orig_opt.num_CSL_samples,
        CSL_loss_frac_cutoff=orig_opt.CSL_loss_frac_cutoff,
        max_CSL_epochs=orig_opt.max_CSL_epochs,
        CSL_loss_weight=orig_opt.CSL_loss_weight,
        CSL_batch_size=orig_opt.CSL_batch_size,
    )
